use diesel::prelude::*;
use diesel_async::RunQueryDsl;
use uuid::Uuid;

use crate::mgmt::dto::error::{AppError, Result};
use crate::mgmt::model::organization::Organization;
use crate::mgmt::repository::db::DbPool;
use crate::mgmt::schema::schema::organizations;

#[derive(Clone)]
pub struct OrganizationRepository {
    pool: DbPool,
}

impl OrganizationRepository {
    pub fn new(pool: DbPool) -> Self {
        Self { pool }
    }

    pub async fn create(&self, id: Uuid, name: &str) -> Result<Organization> {
        let conn = &mut self.pool.get().await
            .map_err(|e| AppError::Internal(e.to_string()))?;
        diesel::insert_into(organizations::table)
            .values((organizations::id.eq(id), organizations::name.eq(name)))
            .get_result(conn)
            .await
            .map_err(Into::into)
    }

    pub async fn find_by_id(&self, id: Uuid) -> Result<Option<Organization>> {
        let conn = &mut self.pool.get().await
            .map_err(|e| AppError::Internal(e.to_string()))?;
        organizations::table
            .filter(organizations::id.eq(id))
            .first::<Organization>(conn)
            .await
            .optional()
            .map_err(Into::into)
    }

    pub async fn list_all(&self) -> Result<Vec<Organization>> {
        let conn = &mut self.pool.get().await
            .map_err(|e| AppError::Internal(e.to_string()))?;
        organizations::table
            .load::<Organization>(conn)
            .await
            .map_err(Into::into)
    }

    pub async fn update(&self, id: Uuid, name: &str) -> Result<Organization> {
        let conn = &mut self.pool.get().await
            .map_err(|e| AppError::Internal(e.to_string()))?;
        diesel::update(organizations::table.filter(organizations::id.eq(id)))
            .set(organizations::name.eq(name))
            .get_result(conn)
            .await
            .map_err(Into::into)
    }

    pub async fn delete(&self, id: Uuid) -> Result<()> {
        let conn = &mut self.pool.get().await
            .map_err(|e| AppError::Internal(e.to_string()))?;
        diesel::delete(organizations::table.filter(organizations::id.eq(id)))
            .execute(conn)
            .await
            .map_err(|_| AppError::Internal("Failed to delete organization".into()))?;
        Ok(())
    }

    pub async fn exists(&self, id: Uuid) -> Result<bool> {
        use diesel::dsl::exists;
        let conn = &mut self.pool.get().await
            .map_err(|e| AppError::Internal(e.to_string()))?;
        diesel::select(exists(
            organizations::table.filter(organizations::id.eq(id))
        ))
        .get_result(conn)
        .await
        .map_err(Into::into)
    }
}
